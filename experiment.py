...
Method:
    ...
    5. Report Results - Pass on false negative and false positive ratio
    
    Random Re-Annotation Sampling:
    6. Split Random Re-Annotation Data
        a. Random Train - 80%
        b. Random Test  - 20%
    ...
    10. Split Intelligent Re-Annotation Data
        a. Intelligent Train - 80%
        b. Intelligent Test  - 20%
    11. Train Intelligent Model
    12. Test Intelligent Model
    13. Report Results
    
    14. Testing the two new methods against the test split for the original test data
...
for trial in range(1, trials):
    ...
    original_test_features = test_data_features.copy()  # for later use
    ...
    # ------ Testing against original test set ------#

    print("\nTesting Random Sampling SVM against original test set")
    predicted = r_clf.predict(original_test_features)

    # Appending Results
    p, r, f, non = precision_recall_fscore_support(original_y_test, predicted, average="weighted")
    r_key = "Randomized-" + str(trial) + "-original"
    results[r_key] = {"Precision": p, "Recall": r, "F-Score": f}

    print("\nTesting Intelligent Sampling SVM against original test set")
    predicted = i_clf.predict(original_test_features)

    # Appending results
    p, r, f, non = precision_recall_fscore_support(original_y_test, predicted, average="weighted")
    i_key = "Intelligent-" + str(trial) + "-original"
    results[i_key] = {"Precision": p, "Recall": r, "F-Score": f}
